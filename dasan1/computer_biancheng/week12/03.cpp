...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	pMonster = new CSprite(hdcMonster, 100, 100, bmMonster.bmWidth, bmMonster.bmHeight);

	// walker
	HDC hdcWalker = CreateCompatibleDC(hdc);
	HBITMAP hBmWalker = (HBITMAP)LoadImage(NULL, L"walker.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	SelectObject(hdcWalker, hBmWalker);
	BITMAP bmWalker;
	GetObject(hBmWalker, sizeof(BITMAP), &bmWalker);
	pWalker = new CAnimationSprite(hdcWalker, 4, 5, 500, 100, bmWalker.bmWidth / 4, bmWalker.bmHeight / 4, 0, 0, bmWalker.bmWidth, bmWalker.bmHeight);
	...
	while (Msg.message != WM_QUIT)
	{
		...
		// 渲染
		if (pMonster->CollideWith(*pWalker))
		{
			if (pMonster->GetBoundingBox()->top <= pWalker->GetBoundingBox()->top)
			{
				pMonster->SetVisible(false); // 设置不可见
			}
			else {
				pWalker->SetVisible(false); // 设置不可见
			}
		}
