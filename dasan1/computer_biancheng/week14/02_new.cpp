...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		for (int i = monsterCount - 1; i > 0; i--)
		{
			...
			{
				index = i;
				pMonster[i]->SetZOrder(monsterCount + walkerCount - 1);
	...
	case WM_KEYDOWN:
		...
		case 'm':
			...
			// monster左上右下移动
		case VK_LEFT:
			pMonster[index]->SetPos(pMonster[index]->GetXPos() - STEP, pMonster[index]->GetYPos());
			break;
		case VK_UP:
			pMonster[index]->SetPos(pMonster[index]->GetXPos(), pMonster[index]->GetYPos() - STEP);
			break;
		case VK_RIGHT:
			pMonster[index]->SetPos(pMonster[index]->GetXPos() + STEP, pMonster[index]->GetYPos());
			break;
		...
		for (int i = 0; i < 10; i++)
		{
			...
			if (pMonster[index]->CollideWith(rtSprite))
			{
