...
HRESULT DDReLoadBitmap(IDirectDrawSurface7 *pdds, LPTSTR szBitmap)
{
    ...
    hr = DDCopyBitmap(pdds, hbm, 0, 0, 0, 0); //拷贝到页面
    if (hr != DD_OK)
    {
...
extern "C" HRESULT DDCopyBitmap(IDirectDrawSurface7 *pdds, HBITMAP hbm, int x, int y, int dx, int dy)
{
    ...
    //
    hdcImage = CreateCompatibleDC(NULL); //与应用程序屏幕相对应的内存DC
    if (!hdcImage)
    ...
    // get size of the bitmap
    //
    GetObject(hbm, sizeof(bm), &bm);    // get size of bitmap 取得图像的尺寸
    dx = dx == 0 ? bm.bmWidth  : dx;    // use the passed size, unless zero
    dy = dy == 0 ? bm.bmHeight : dy;    // 如果给出的宽和高为零，则取图像的宽和高
    ...
    pdds->GetSurfaceDesc(&ddsd);  //取得表面的属性：宽和高

    if ((hr = pdds->GetDC(&hdc)) == DD_OK) //获得hdc锁住表面
    {
        //拷贝数据
		StretchBlt(hdc, 0, 0, ddsd.dwWidth, ddsd.dwHeight, hdcImage, x, y, dx, dy, SRCCOPY);
        pdds->ReleaseDC(hdc); //释放表面，解锁
    }
