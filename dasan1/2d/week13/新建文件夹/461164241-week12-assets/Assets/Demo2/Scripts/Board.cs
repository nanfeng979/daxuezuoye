...
public class Board : MonoBehaviour
{
    ...
    public List<CardMgr> cardList = new List<CardMgr>(); 
    ...
    {
        if (point.x <0 || point.x >= row || point.y>= col || point.y<0)
            return false;
    ...
    private void CreateBlock(int x, int y)
    {
        ...
        var nodeObj = realNodeObj.GetComponent<CardMgr>();
        this.cardList.Add(nodeObj);
...
//        Debug.Log($"TRY MOVE {from.point} , {to.point}");
    ...
    public void UpdateState()
    {
        ...
        if (targetAllNull)
        {
            ...
            { 
                var removed = new List<CardMgr>();
                List<CardMgr> removeTarget = new List<CardMgr>();
                foreach (var data in cardList) 
                    if (data.needDestroy)  
